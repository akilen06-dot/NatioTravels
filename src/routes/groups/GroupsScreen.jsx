    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded-full bg-accent-tint px-2.5 py-1 text-[12px] font-medium text-accent-strong">
          {t('{count} going', { count: group.members.length })}
        </span>
      </div>
  ...
  if (!hasAccess(currentUser)) {
    return currentUser?.plan ? (
      <TripLockedNotice
        ...
        body={t('Trip passes cover up to 14 days. Renew your plan to see group meetups and locations again.')}
      />
    ) : (
      <TripLockedNotice
        title={t('Groups are for paying travelers')}
        body={t('Add a Trip Pass or subscription to browse, join, and create group meetups.')}
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-ink">{t('Groups')}</h1>
          <p className="mt-1 text-[13.5px] text-ink-muted">{t('Meet up with travelers from home.')}</p>
        </div>
        <Link
          to="/groups/new"
      ...
      {mine.length > 0 && (
        <section className="mt-7">
          <h2 className="text-[13px] font-medium text-ink-muted">{t('My groups')}</h2>
      ...
      <section className="mt-8">
        <h2 className="text-[13px] font-medium text-ink-muted">{t('Find a group by nationality')}</h2>
        <div className="relative mt-3">
          ...
          <input
            ...
            placeholder={t('Search nationality, like Brazil')}
        ...
        <div className="mt-4 flex flex-col gap-3">
          {discoverable.length === 0 ? (
            <div className="flex flex-col items-center rounded-2xl border border-dashed border-border-strong py-10 text-center">
              ...
              <p className="mt-2 text-[13.5px] text-ink-muted">{t('No groups match that search yet.')}</p>
            </div>
          ) : (
