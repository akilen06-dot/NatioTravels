...
export default function SignIn() {
  ...
  return (
    <OnboardingLayout title={t('Welcome back')} subtitle={t('Sign in to continue.')}>
      <form
        onSubmit={handleSubmit}
        className="flex flex-col gap-5"
        onChange={() => signInError && clearSignInError()}
      >
        <Field label={t('Email or username')} htmlFor="signin-identifier" error={signInError}>
          <input
        ...
        <Field label={t('Password')} htmlFor="signin-password">
      ...
      <div className="mt-8 rounded-2xl border border-border bg-bg-sunken p-4">
        <p className="text-[13px] font-medium text-ink-muted">
          {t('Prototype demo accounts · password is')}{' '}
