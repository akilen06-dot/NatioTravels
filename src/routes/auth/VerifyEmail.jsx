...
export default function VerifyEmail() {
  ...
  return (
    <OnboardingLayout title={t('Email verification')} onBack={() => navigate(continuePath)}>
      <div className="flex flex-col items-center rounded-2xl border border-border bg-bg-sunken p-7 text-center">
        {status === 'checking' && <p className="text-[14.5px] text-ink-muted">{t('Checking…')}</p>}
        ...
        {status === 'verified' && (
          <>
            ...
            <p className="mt-4 text-[15px] font-medium text-ink">{t('Email confirmed')}</p>
            <p className="mt-1.5 text-[13px] text-ink-muted">{t('Thanks — your email address is now verified.')}</p>
          </>
        )}
        ...
        {status === 'invalid' && (
          <>
            ...
            <p className="mt-4 text-[15px] font-medium text-ink">{t('Link expired or already used')}</p>
            <p className="mt-1.5 text-[13px] text-ink-muted">
              {t('You can request a new verification email from your profile settings.')}
            </p>
          </>
        )}
