...
export default function DiscoverScreen() {
  ...
  if (confirmingPayment) {
    return (
      <div className="flex h-full items-center justify-center px-5 py-8">
        <p className="text-[13.5px] text-ink-muted">{t('Confirming your payment…')}</p>
      </div>
    )
  }
  ...
  if (!hasAccess(currentUser)) {
    return currentUser?.plan ? (
      ...
      <TripLockedNotice
        title={t('Discover is for paying travelers')}
        body={t('Add a Trip Pass or subscription to browse and match with travelers of your nationality.')}
  ...
  if (swipeLimitReached) {
    return (
      <TripLockedNotice
        title={t("You're out of swipes for this week")}
        body={t('Free access is capped at {limit} swipes a week. It resets on a rolling basis, or add a plan for unlimited swiping.', { limit: PROMO_WEEKLY_SWIPE_LIMIT })}
        ctaLabel={t('Add a plan to unlock')}
      />
    )
  }
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center px-5 py-8">
      <div className="relative w-full text-center">
        <h1 className="text-xl font-semibold text-ink">{t('Discover')}</h1>
        ...
        <button
          ...
          disabled={refreshing}
          aria-label={t('Review skipped travelers')}
          title={t("Bring back people you've skipped")}
      ...
      <div className="relative mt-6 h-[480px] w-full max-w-[340px]">
        {deck.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center rounded-3xl border border-dashed border-border-strong text-center">
            <p className="text-[15px] font-medium text-ink">{t("You're all caught up")}</p>
            <p className="mt-1 max-w-[220px] text-[13.5px] text-ink-muted">
              {t('Check back soon for more travelers nearby.')}
            </p>
          </div>
        ) : (
          deck
