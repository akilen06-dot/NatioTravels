...
export default function BlockedReported() {
  ...
  return (
    <div className="mx-auto max-w-lg px-5 py-8">
      ...
      <h1 className="mt-4 text-xl font-semibold text-ink">{t('Blocked & reported')}</h1>
      <p className="mt-1.5 text-[13.5px] leading-relaxed text-ink-muted">
        {t('Block or report anyone from the flag icon on their profile.')}
      </p>

      <section className="mt-6">
        <h2 className="text-[13px] font-medium text-ink-muted">{t('Blocked')}</h2>
        {blockedIds.length === 0 ? (
          <div className="mt-3 flex flex-col items-center rounded-2xl border border-dashed border-border-strong py-10 text-center">
            ...
            <p className="mt-2 text-[13.5px] text-ink-muted">{t("You haven't blocked anyone.")}</p>
          </div>
        ) : (
      ...
      <section className="mt-7">
        <h2 className="text-[13px] font-medium text-ink-muted">{t('Reported')}</h2>
        {reports.length === 0 ? (
          <div className="mt-3 flex flex-col items-center rounded-2xl border border-dashed border-border-strong py-10 text-center">
            <Flag size={26} className="text-ink-faint" />
            <p className="mt-2 text-[13.5px] text-ink-muted">{t("You haven't reported anyone.")}</p>
          </div>
        ) : (
          <div className="mt-2 divide-y divide-border rounded-2xl border border-border bg-bg-raised px-4">
            {reports.map((r) => (
      ...
      <p className="mt-6 flex items-center gap-1.5 text-[12.5px] text-ink-faint">
        <ShieldSlash size={14} />
        {t("Blocked people can't message you or see your profile in Discover or Search.")}
      </p>
    </div>
  )
}
