...
export default function PrivacySettings() {
  ...
  return (
    <div className="mx-auto max-w-lg px-5 py-8">
      ...
      <h1 className="mt-4 text-xl font-semibold text-ink">{t('Account privacy')}</h1>
      ...
      <div className="mt-6 rounded-2xl border border-border bg-bg-raised p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[14.5px] font-medium text-ink">{t('Private account')}</p>
              <p className="mt-1 text-[13px] leading-relaxed text-ink-muted">
                {t("When your account is private, other travelers can't view your profile, posts, or trip details unless you've already matched with them. Your profile still appears in Discover and Search.")}
              </p>
            </div>
          </div>
          <Switch
      ...
      {currentUser.private && (
        <p className="mt-3 px-1 text-[12.5px] text-ink-muted">
          {t("Your account is currently private. People you haven't matched with will see a locked profile.")}
        </p>
      )}
    </div>
  )
