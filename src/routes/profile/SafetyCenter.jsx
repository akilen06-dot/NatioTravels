...
export default function SafetyCenter() {
  ...
  return (
    <div className="mx-auto max-w-lg px-5 py-8">
      ...
      <h1 className="mt-4 text-xl font-semibold text-ink">{t('Safety & privacy')}</h1>
      <p className="mt-1.5 text-[13.5px] leading-relaxed text-ink-muted">
        {t('How Natio keeps your location, payments, and meetups safe.')}
      </p>
      ...
      <section className="mt-4 rounded-2xl border border-border bg-bg-raised p-5">
        <div className="flex items-center gap-2.5">
          ...
          <p className="text-[14.5px] font-medium text-ink">{t('Blocked & reported users')}</p>
        </div>
        <p className="mt-2 text-[13.5px] leading-relaxed text-ink-muted">
          {t("You haven't blocked anyone yet. You can block or report someone from their profile or a conversation at any time.")}
        </p>
      </section>
      ...
      <section className="mt-4 rounded-2xl border border-danger/30 bg-danger-tint p-5">
        <div className="flex items-center gap-2.5">
          ...
          <p className="text-[14.5px] font-medium text-ink">{t('Delete my data')}</p>
        </div>
        <p className="mt-2 text-[13.5px] leading-relaxed text-ink-muted">
          {t("Permanently deletes your profile, verification photos, matches, and messages. This can't be undone.")}
        </p>
        ...
        {!confirming ? (
          ...
          <div className="mt-4 flex flex-col gap-2.5 rounded-xl border border-danger/40 bg-bg-raised p-4">
            <p className="text-[13.5px] font-medium text-ink">
              {t('Are you sure? This permanently deletes your account.')}
            </p>
