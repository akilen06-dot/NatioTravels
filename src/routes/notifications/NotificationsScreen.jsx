...
import { useT, useTimeAgo, describeNotification } from '../../lib/i18n'
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-[14px] text-ink">{describeNotification(t, notif, actor?.name || t('Someone'))}</p>
      </div>
...
export default function NotificationsScreen() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-5 py-8">
      <h1 className="text-xl font-semibold text-ink">{t('Notifications')}</h1>
      ...
      {notifications.length === 0 ? (
        <div className="mt-10 flex flex-col items-center rounded-2xl border border-dashed border-border-strong py-14 text-center">
          ...
          <p className="mt-2 text-[13.5px] text-ink-muted">
            {t('Likes, matches, and messages will show up here.')}
          </p>
        </div>
      ) : (
