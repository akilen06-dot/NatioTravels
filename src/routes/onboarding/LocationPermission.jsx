...
export default function LocationPermission() {
  ...
  return (
    <OnboardingLayout
      step={2}
      totalSteps={4}
      title={t('Turn on location')}
      subtitle={t('We use your location to show nearby travelers of your nationality. Your exact position is never shared, only an approximate distance.')}
    >
      <div className="flex flex-col items-center rounded-2xl border border-border bg-bg-sunken p-7 text-center">
        ...
        <p className="mt-4 text-[15px] font-medium text-ink">{t('Allow "Natio" to use your location?')}</p>
        <p className="mt-1.5 text-[13.5px] text-ink-muted">
          {t("Only while you're using the app.")}
        </p>

        {status === 'idle' ? (
          <div className="mt-5 flex w-full flex-col gap-2.5">
            <Button onClick={handleAllow} className="w-full">
              {t('Allow while using app')}
            </Button>
          ...
          <p className="mt-5 text-[13.5px] font-medium text-ink-muted">{t('Requesting location…')}</p>
        ) : status === 'granted' ? (
          <p className="mt-5 text-[13.5px] font-medium text-success">{t('Location enabled')}</p>
        ) : (
          <p className="mt-5 text-[13.5px] font-medium text-ink-muted">{t('Continuing without location')}</p>
        )}
      </div>
    </OnboardingLayout>
  )
